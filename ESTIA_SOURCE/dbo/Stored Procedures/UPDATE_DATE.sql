...
-- =============================================
CREATE PROCEDURE [dbo].[UPDATE_DATE]
AS
BEGIN
	...
	WHERE c.COLUMN_NAME LIKE 'D%'
	AND c.TABLE_NAME <> 'L_RECUPERATION_ESTIA'
	AND (c.TABLE_NAME NOT IN ('BASELOC') AND c.COLUMN_NAME <> 'DEFFET')
	AND (c.TABLE_NAME NOT IN ('HQRBAIL') AND c.COLUMN_NAME <> 'DRENOUV')
	AND (c.TABLE_NAME NOT IN ('PDEPOP') AND c.COLUMN_NAME <> 'DUREE')
	...
	-- construire le texte du traitement 
	   SET @SQL = 'UPDATE ' + @TBL + ' SET ' + @COL + ' = NULL WHERE ' + @COL + ' = '''''
	   PRINT @SQL
