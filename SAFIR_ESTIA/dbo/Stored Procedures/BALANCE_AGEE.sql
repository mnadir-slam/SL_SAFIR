...
(
	SELECT PK_PROPRIETAIRE
		 ...
		 , CASE WHEN DT_SORTIE IS NULL OR DT_SORTIE >= @DT_DEF THEN 'ACTIFS NON SOLDES' WHEN DT_SORTIE < @DT_DEF THEN 'SORTIS NON SOLDES' END ETAT
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) <= 30 THEN MT_SOLDE ELSE 0 END AS INT_30
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) BETWEEN 31 AND 60 THEN MT_SOLDE ELSE 0 END AS INT_31_60
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) BETWEEN 61 AND 90 THEN MT_SOLDE ELSE 0 END AS INT_61_90
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) BETWEEN 91 AND 180 THEN MT_SOLDE ELSE 0 END AS INT_91_180
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) BETWEEN 181 AND 365 THEN MT_SOLDE ELSE 0 END AS INT_181_365
		 , CASE WHEN DATEDIFF(DAY, DT_TERME, CONVERT(DATETIME, @DT_DEF, 103)) > 365 THEN MT_SOLDE ELSE 0 END AS INT_365
