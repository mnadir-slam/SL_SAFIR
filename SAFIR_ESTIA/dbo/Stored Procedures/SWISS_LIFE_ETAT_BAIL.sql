...
-- =============================================
CREATE PROCEDURE [dbo].[SWISS_LIFE_ETAT_BAIL] 
(	
	@PK_TEMPS DATETIME,
	@PK_IMMEUBLE VARCHAR,
	@PK_PROPRIETAIRE VARCHAR,
	@PK_LOCATAIRE VARCHAR
)
AS
BEGIN
...
(
	SELECT FEL.FK_PROPRIETAIRE
		 ...
		 , DB.DT_SIGNATURE
		 , DB.DT_EFFET
                                 , DATEADD(MONTH, (DB.DUREE_AN*12) + DB.DUREE_MOIS, DB.DT_EFFET)-1AS DT_FIN
		 , DB.DT_ENTREE
		 , DB.DT_CONGE
		 , DB.DT_SORTIE
		 , DB.DT_PRCH_TRIEN_POT
		 , DATEADD(month, -NB_MOIS_PREAVIS, DT_PRCH_TRIEN_POT) DT_RISQUE
		 , FEL.CCOMPTE
		 , FEL.CD_STATUT 
                                 , FEL.LB_STATUT
		 , DB.LB_TYPE_CAUTION
		 ...
		 , DB.DT_FIN_FRANCHISE
		 , VLB.MT_LOYER_BASE
		 , CAST(0 AS NUMERIC(18,2)) AS MT_DG_BASE
		 ...
		 , FEL.DT_INDICE_ACTUEL
                                 , FEL.CD_INDICE_REVISION AS TYPE_INDICE
	...
	WHERE
	  ...
	  AND FEL.FK_IMMEUBLE IN (@PK_IMMEUBLE)
	  AND ISNULL(FEL.FK_LOCATAIRE, 99999) IN (@PK_LOCATAIRE)
...
SELECT REQ.FK_PROPRIETAIRE
	 ...
	 , REQ.DT_CONGE
	 , REQ.DT_SORTIE
	 , REQ.DT_PRCH_TRIEN_POT
	 , REQ.DT_RISQUE
	 , CASE WHEN REQ.DT_FIN < GETDATE() THEN 'O' ELSE 'N' END AS IND_ECHU
	 , REQ.CCOMPTE
	 , REQ.CD_STATUT 
                 , REQ.LB_STATUT
	 , REQ3.NB_PARKINGS
	 ...
	 , REQ.MT_LOYER_BASE
	 , REQ.MT_DG_BASE
                 , (CASE WHEN REQ.VAL_INDICE_BASE = 0 THEN 0 ELSE(REQ.MT_LOYER_BASE*REQ.VAL_INDICE_REVISION)/REQ.VAL_INDICE_BASE END) AS MT_LOYER_INDEXE
		, (CASE WHEN REQ.VAL_INDICE_BASE = 0 THEN 0 ELSE (REQ.MT_DG*REQ.VAL_INDICE_REVISION)/REQ.VAL_INDICE_BASE END) AS MT_DG_INDEXE
	 , REQ.MT_LOYER_ACTUEL
	 , REQ.MT_DG
	 , REQ.MT_CHARGES
...
(
		...
		(
			...
			FROM D_LOT L
			WHERE FK_IMMEUBLE IN (@PK_IMMEUBLE)
			AND VAL_SURFACE_REELLE > 0
		) LOT
		ON LOT.PK_LOT = FEL.PK_LOT
		WHERE FEL.PK_TEMPS = @PK_TEMPS AND FEL.FK_IMMEUBLE IN (@PK_IMMEUBLE)
