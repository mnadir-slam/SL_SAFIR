...
-- =============================================
CREATE PROCEDURE [dbo].[COMPTABILITE_LOCATAIRE_DET_SALEFORCE] 
(
	@DT_DEBUT DATETIME,
	@DT_FIN DATETIME,
	--@PK_PROPRIETAIRE INT,
...
LEFT JOIN MASTER_ESTIA..P_TRANSCO_IMMEUBLE_EASYFOLDER PT
    ON I.NM_IMMEUBLE = PT.NM_IMMEUBLE_ESTIA
WHERE FCL.DT_COMPTABLE BETWEEN @DT_DEBUT AND @DT_FIN
AND FCL.DT_TERME <= @DT_FIN
--AND P.PK_PROPRIETAIRE = @PK_PROPRIETAIRE
--AND I.PK_IMMEUBLE IN (SELECT VALUE FROM dbo.ufn_SplitMultiValue(@PK_IMMEUBLE,','))
AND PT.CD_IMMEUBLE_DREAM IN (SELECT VALUE FROM dbo.ufn_SplitMultiValue(@CD_IMMEUBLE_DREAM,','))
