...
SELECT IDENTIFIANT_BAIL_DREAM
	 ...
	 --, CASE WHEN LOCATAIRE_PRINCIPAL LIKE '%Total%' THEN SURFACE_TOTALE ELSE SUM(VAL_SURF_BAIL) END VAL_SURF_BAIL
...
(
	SELECT D_BAIL.PK_BAIL
	 ...
	 , NULL AS DT_EFFET
	 , NULL AS DT_FIN
     , ISNULL(VAL_FIN.MT_LOYER_ACTUEL, 0) AS LOYER_ANNUEL_FACIAL
     , ISNULL(VAL_FIN.MT_LOYER_ACTUEL_FACTURABLE - ABS(VAL_FIN.MT_LOYER_FRANCHISE) - ABS(VAL_FIN.MT_PALIER), 0) AS DERNIER_TERME_ANNUALISE
...
FROM
  ...
  INNER JOIN
	...
	on CALCUL_SURFACE.FK_BAIL = D_BAIL.PK_BAIL
WHERE
