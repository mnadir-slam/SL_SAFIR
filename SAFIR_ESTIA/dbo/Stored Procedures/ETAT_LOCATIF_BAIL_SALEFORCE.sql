...
-- =============================================
CREATE PROCEDURE [dbo].[ETAT_LOCATIF_BAIL_SALEFORCE] 
(
	@PK_TEMPS DATETIME,
...
(
	...
	ON DI.NM_IMMEUBLE = P.NM_IMMEUBLE_ESTIA AND DI.CD_SOCIETE = '11'
	WHERE
...
(
	SELECT REQ.DT_REF
		 ...
		 , REQ.CD_IMMEUBLE 
		 , REQ.NM_IMMEUBLE
		 , P_TRANSCO_IMMEUBLE_EASYFOLDER.CD_IMMEUBLE_DREAM
		 , REQ.ADRLGN1
		 , REQ.SURFACE_TOTALE
		 , REQ.VLM_ARCHIVE
		 ...
		 , REQ.NM_LOCATAIRE
		 , REQ.NOM_LOCATAIRE
		 , CASE WHEN REQ.NOM_LOCATAIRE =  'Vacant' THEN 'Vacant '+P_TRANSCO_IMMEUBLE_EASYFOLDER.CD_IMMEUBLE_DREAM ELSE REQ.NOM_LOCATAIRE END NOM_LOCATAIRE_DREAM
		 , REQ.CD_LOT_PRINCIPAL
		 , REQ.CD_BAIL
		 , REQ.NM_BAIL
		 , CASE WHEN REQ.CD_STATUT = 'L' THEN CASE WHEN ABS(REQ.FK_LOCATAIRE) = 88888 THEN 'BAIL999997' ELSE EASYFOLDER_BAUX.IDENTIFIANT_BAIL_DREAM END ELSE 'BAIL999998' END IDENTIFIANT_BAIL_DREAM
		 , REQ.LB_NATURE_BAIL
		 , REQ.LB_DUREE
		 , REQ.DT_SIGNATURE
		 ...
		 , W.VAL_SURF_HAB
		 , W.VAL_SURF_HOT
		 , W.VAL_SURF_BUR + W.VAL_SURF_COM + W.VAL_SURF_ACT + W.VAL_SURF_ARC + W.VAL_SURF_HAB + W.VAL_SURF_HOT AS VAL_SURF_BAIL
		 , CASE WHEN REQ.SURFACE_TOTALE > 0 THEN (W.VAL_SURF_BUR + W.VAL_SURF_COM + W.VAL_SURF_ACT + W.VAL_SURF_ARC + W.VAL_SURF_HAB + W.VAL_SURF_HOT)/REQ.SURFACE_TOTALE ELSE 0 END AS PC_SURF_IMMEUBLE
		 , REQ3.ETAGE
		 , REQ.LB_TYPE_CAUTION
		 ...
		 , (REQ.MT_LOYER_ACTUEL - REQ.MT_LOYER_FRANCHISE - REQ.MT_PALIER) MT_LOYER_ACTUEL_FACTURE
		 , REQ.MT_DG
		 , REQ.MT_CHARGES
	...
	LEFT JOIN MASTER_ESTIA..P_TRANSCO_IMMEUBLE_EASYFOLDER
	ON P_TRANSCO_IMMEUBLE_EASYFOLDER.NM_IMMEUBLE_ESTIA = REQ.NM_IMMEUBLE
	LEFT JOIN MASTER_ESTIA..L_INTERFACE_EASYFOLDER_BAUX EASYFOLDER_BAUX
	ON REQ.NM_IMMEUBLE = EASYFOLDER_BAUX.IDENTIFIANT_ESTIA_IMMEUBLE AND REQ.NM_BAIL = CAST(EASYFOLDER_BAUX.IDENTIFIANT_ESTIA_COMPTE AS VARCHAR(30)) + DBO.LPAD(EASYFOLDER_BAUX.NOBAIL_ESTIA, 2, '0')
	LEFT JOIN
	(
			...
			(
				SELECT L.PK_LOT, L.CD_SOCIETE, L.NM_IMMEUBLE, L.NM_LOT, L.CD_USAGE, L.CD_REG_USAGE, L.LB_REG_USAGE, L.LB_USAGE, VAL_SURFACE_REELLE  
				FROM D_LOT L
				LEFT JOIN D_IMMEUBLE DI
				ON DI.PK_IMMEUBLE = L.FK_IMMEUBLE
				LEFT JOIN MASTER_ESTIA..P_TRANSCO_IMMEUBLE_EASYFOLDER P
				ON DI.NM_IMMEUBLE = P.NM_IMMEUBLE_ESTIA AND DI.CD_SOCIETE = '11'
			...
			WHERE FEL.PK_TEMPS = @PK_TEMPS 
			AND P.CD_IMMEUBLE_DREAM IN (SELECT VALUE FROM dbo.ufn_SplitMultiValue(@CD_IMMEUBLE_DREAM,','))
			--AND (LOT.CD_SOCIETE + CAST(LOT.NM_IMMEUBLE AS VARCHAR(10)) + CAST(LOT.NM_LOT AS VARCHAR(10)) <> '1159023001')
...
(
	SELECT distinct DT_REF
		 ...
		 , VLM_PARKING_DOUBLE
		 , VLM_PARKING_SIMPLE
		 , NULL AS VAL_SURFACE_REELLE
		 , NULL AS FK_LOCATAIRE
		 , NULL AS CD_LOCATAIRE
		 , NULL AS NM_LOCATAIRE
		 , 'Vacant' AS NOM_LOCATAIRE
		 , CONCAT('Vacant ', CD_IMMEUBLE_DREAM) AS NOM_LOCATAIRE_DREAM
		 , NULL AS CD_LOT_PRINCIPAL
		 , NULL AS CD_BAIL
		 , NULL AS NM_BAIL 
		 , 'BAIL999998' AS IDENTIFIANT_BAIL_DREAM
		 , NULL AS LB_NATURE_BAIL
		 , NULL AS LB_DUREE
		 , NULL AS DT_SIGNATURE
		 , NULL AS DT_EFFET
		 , NULL AS DT_FIN
		 , NULL AS DT_ENTREE
		 , NULL AS DT_CONGE
		 , NULL AS DT_SORTIE
		 , NULL AS DT_PRCH_TRIEN_POT
		 , NULL AS DT_RISQUE
		 , 'N' AS IND_ECHU
		 , NULL AS CCOMPTE
		 , 'V' AS CD_STATUT 
		 , 'Vacant' AS LB_STATUT
		 , NULL AS NB_PARKINGS
		 , NULL AS NB_PARKINGS_SIMPLES
		 , NULL AS NB_PARKINGS_DOUBLES
		 , NULL AS VAL_SURF_BUR
		 , NULL AS VAL_SURF_COM
		 , NULL AS VAL_SURF_ACT
		 , NULL AS VAL_SURF_ARC
		 , NULL AS VAL_SURF_HAB
		 , NULL AS VAL_SURF_HOT
		 , NULL AS VAL_SURF_BAIL
		 , NULL AS PC_SURF_IMMEUBLE
		 , NULL AS ETAGE
		 , NULL AS LB_TYPE_CAUTION
		 , NULL AS DT_DEBUT_FRANCHISE
		 , NULL AS DT_FIN_FRANCHISE
		 , NULL AS MT_LOYER_BASE
		 , NULL AS MT_LOYER_INDEXE
		 , NULL AS MT_DG_INDEXE
		 , NULL AS MT_LOYER_ACTUEL
		 , NULL AS MT_LOYER_ACTUEL_FACTURABLE
		 , NULL AS MT_LOYER_ACTUEL_FACTURE
		 , NULL AS MT_DG
		 , NULL AS MT_CHARGES
		 , NULL AS MT_LOYER_FRANCHISE
		 , NULL AS MT_PALIER
		 , NULL AS LOYER_REVISE_SUR_LOYER_BASE
		 , NULL AS LB_INDICE_INITIAL
		 , NULL AS DT_INDICE_INITIAL
		 , NULL AS LB_INDICE_ACTUEL
		 , NULL AS DT_INDICE_ACTUEL
		 , NULL AS TYPE_INDICE
		 , NULL AS INDICE_BASE
		 , NULL AS VAL_INDICE_BASE
		 , NULL AS DT_DERNIERE_REVISION
		 , NULL AS INDICE_REVISION
		 , NULL AS VAL_INDICE_REVISION
		 , NULL AS PC_RECUP_TF
		 , NULL AS PC_RECUP_HO
		 , NULL AS PC_RECUP_TB
		 , NULL AS PC_RECUP_AS
		 , NULL AS PC_RECUP_606
		 , NULL AS PC_RECUP_TOM
		 , NULL AS NB_AVENANT
		 , NULL AS A_SUIVRE
		 , NULL AS VAL_SURF_UTILE_GEOMETRE
		 , NULL AS VLM
		 , NULL AS LOYER_REVISE_SUR_VLM
		 , NULL AS RISQUE_DEPART
		 , NULL AS COMMENTAIRES
		 , NULL AS CHEMIN_BAIL
		 , NULL AS CHEMIN_PLAN
		 , NULL AS CHEMIN_ETAT_SURFACES
		 , NULL AS CHEMIN_TB
		 , NULL AS CHEMIN_TF
		 , NULL AS PAIEMENT
  		 , NULL AS CHEMIN_MANDAT
	...
	(
		SELECT *
		FROM REQ4 B
		WHERE B.CD_STATUT = 'V' AND B.FK_IMMEUBLE = A.FK_IMMEUBLE
	)
)
