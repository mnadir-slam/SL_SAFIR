...
DELETE FROM WRK_INTERFACE_EASYFOLDER_CONTRATS WHERE IFC_STATUT_TRAITEMENT IS NULL;
...
SELECT CONVERT(DATETIME, CONVERT(VARCHAR(10),GETDATE(),103), 103) AS PK_TEMPS
	 , 'ADD' AS ACTION
...
(
	SELECT P.CD_IMMEUBLE_DREAM AS IDENTIFIANT_DREAM_IMMEUBLE
		 ...
		 --, CONCAT(CONCAT(T.LTIERS, + ' - ' + CONVERT(VARCHAR(10), CAST(CTR.CCONTFRS AS INT)) + CASE WHEN CTR.LIBFAC IS NOT NULL THEN ' - ' + CTR.LIBFAC END), + CASE WHEN CTR.LCTRTIERS IS NOT NULL THEN ' - ' + CTR.LCTRTIERS END) AS LB_CONTRAT
		 , CONCAT(CONCAT(CONCAT(T.LTIERS, ' - ' + CONVERT(VARCHAR(10), CAST(CTR.CCONTFRS AS INT))), ' - ' + CTR.LIBFAC), ' - ' + CTR.LCTRTIERS) AS LB_CONTRAT
...
(
	SELECT * FROM L_INTERFACE_EASYFOLDER_CONTRATS L
	WHERE L.NUM_CONTRAT = WRK_INTERFACE_EASYFOLDER_CONTRATS.NUM_CONTRAT
)
