...
SELECT     GETDATE() AS DT_REF, LOCATION.PK_LOCATION AS PK_BAIL, TIERS.PK_TIERS AS FK_LOCATAIRE, MANPROP.FK_TIERS AS FK_PROPRIETAIRE, 
                      ...
                      * 100 + LOCATION.NOBAIL END AS NM_BAIL, REPLACE(STR(LOCATION.CGROUPE, 4, 0), ' ', '0') AS CD_IMMEUBLE, 
					  ...
					  -- Debut modif MNA 300119
					  ISNULL(HQ.DRENOUV, CASE WHEN DATEPART(YEAR, LOCATION.DATEFFET) < 1900 THEN '19000101' ELSE LOCATION.DATEFFET END) AS DT_EFFET,
                      ISNULL(WH.DT_EXPIRATION, DATEADD(day, - 1, DATEADD(month, LOCATION.DURBAIL, ISNULL(HQ.DRENOUV, LOCATION.DATEFFET)))) AS DT_EXPIRATION, 
					  -- Fin modif MNA 300119
...
FROM         dbo.ESTIA_LOCATION AS LOCATION LEFT OUTER JOIN
					...
					(
						select H.CORG
							 , H.CCOMPTE
							 , H.NOBAIL
							 , MAX(H.DRENOUV) DRENOUV
						from ESTIA_HQRBAIL H
						left join ESTIA_LOCATION L
						on L.CCOMPTE = H.CCOMPTE and L.NOBAIL = H.NOBAIL
						left join ESTIA_BAIL B
						on B.CBAIL = L.CBAIL
						left join P_NATURE_BAIL P
						on P.CD_NATURE_BAIL = B.CBAIL
						where H.CORG = '11'
						and P.CD_TYPE_BAIL <> 'HAB'
						and H.DRENOUV is not null
						and H.DRENOUV > '19000101'
						group by H.CORG
							 , H.CCOMPTE
							 , H.NOBAIL
					) HQ
					ON HQ.CORG = LOCATION.CORG AND HQ.CCOMPTE = LOCATION.CCOMPTE AND HQ.NOBAIL = LOCATION.NOBAIL
